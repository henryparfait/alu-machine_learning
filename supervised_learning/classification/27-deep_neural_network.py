...
class DeepNeuralNetwork:
    ...
    def forward_prop(self, X):
        ...
        Uses sigmoid for hidden layers and softmax for the output.

        Args:
            X (numpy.ndarray): Input data with shape (nx, m).

        Returns:
            tuple: (output, cache) - final activation and cache.
        """
        self.__cache["A0"] = X
    ...
    def cost(self, Y, A):
        """
        Calculate the cost using cross-entropy for multiclass.
    ...
    def evaluate(self, X, Y):
        ...
        Returns:
            tuple: (prediction, cost) where prediction is one-hot
            encoded with shape (classes, m).
        """
